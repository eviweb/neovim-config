...
- avante: OAuth prompt at every Neovim startup — switched plugin loading from `VeryLazy` to `cmd`/`keys`; config wrapped in `vim.schedule()` to defer past lazy.nvim's load callback context (avoids E565 "window creation forbidden")
...
- avante: `<Leader>aP` firing native `P` (paste) — lazy.nvim re-fires the triggering key after `vim.schedule`; fixed by declaring a stub keymap in the plugin `keys` table so lazy registers it immediately
- avante: gemini-cli demanding an API key — avante's built-in config forces `auth_method = "gemini-api-key"`; overridden via `acp_providers["gemini-cli"]` without `auth_method` so the gemini CLI uses Google account auth
