...
- avante: claude-code stuck on "generating" — `@zed-industries/claude-code-acp` deprecated (ENOENT); replaced with `@agentclientprotocol/claude-agent-acp` declared directly in `acp_providers` of `setup()` with a 3-element `args` array matching the avante default length, so `tbl_deep_extend` replaces all indices correctly
